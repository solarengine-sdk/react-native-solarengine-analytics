...
function buildDelayDeeplinkResponse():delayDeeplink{
  const handleDelayDeeplink: delayDeeplink = (code:number,delaydeeplink?:DelayDeepLinkInfo) => {
    if(code === 0){
      if(delaydeeplink){
        log("delaydeeplink.sedpLink: " + delaydeeplink.sedpLink);
      }
    }else{
      log("code: " + code);
    }  
...
let preProperties = {"trackCustomEvent_key":" trackCustomEvent_properties_singleValue"}
